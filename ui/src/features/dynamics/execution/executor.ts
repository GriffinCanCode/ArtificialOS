    ...
    switch (action) {
      case "navigate":
        ...
        if (!url) {
          ...
          url =
            this.componentState.get("url-input") ||
    ...
    switch (action) {
      case "load_apps":
        ...
        try {
          ...
          const utilitiesApps = (data.apps || []).filter(
            (app: any) => app.category === "utilities"
          );
  ...
  private async executeFilesystemTool(action: string, params: Record<string, any>): Promise<any> {
    switch (action) {
      case "list":
        ...
        let path =
          params.path ||
          this.componentState.get("path-input") ||
          this.componentState.get("current-path") ||
          "/tmp/ai-os-storage";
