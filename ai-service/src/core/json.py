...
def parse(text: str, *, strict: bool = True, repair: bool = False) -> Result[dict[str, Any], ParseError]:
    """
...
def validate_size(data: str, max_bytes: int) -> Result[None, ParseError]:
    ...
    if size > max_bytes:
        return Failure(
            ParseError(f"Size {size} bytes exceeds maximum {max_bytes} bytes")
        )
    return Success(None)
...
def validate_depth(obj: Any, max_depth: int = 20, _depth: int = 0) -> Result[None, ParseError]:
    ...
    if _depth > max_depth:
        return Failure(
            ParseError(f"Nesting depth {_depth} exceeds maximum {max_depth}")
        )
