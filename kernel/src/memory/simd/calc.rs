...
unsafe fn min_u64_avx512(data: &[u64]) -> u64 {
    ...
    let mut min_val = min_arr.iter().min().copied().unwrap_or(u64::MAX); // Safe fallback, array has 8 elements

    // Handle remaining elements
    if offset < len {
        min_val = min_val.min(data[offset..].iter().min().copied().unwrap_or(u64::MAX));
        // Safe fallback for remainder
    }
...
unsafe fn min_u64_avx2(data: &[u64]) -> u64 {
    ...
    for chunk in data.chunks_exact(4) {
        ...
        min_val = min_val.min(vals.iter().min().copied().unwrap_or(u64::MAX)); // Safe fallback, chunk has 4 elements
    }
...
unsafe fn max_u64_avx512(data: &[u64]) -> u64 {
    ...
    let mut max_val = max_arr.iter().max().copied().unwrap_or(0); // Safe fallback, array has 8 elements

    // Handle remaining elements
    if offset < len {
        max_val = max_val.max(data[offset..].iter().max().copied().unwrap_or(0));
        // Safe fallback for remainder
    }
...
unsafe fn max_u64_avx2(data: &[u64]) -> u64 {
    ...
    for chunk in data.chunks_exact(4) {
        ...
        let values = _mm256_loadu_si256(ptr);
        let vals: [u64; 4] = std::mem::transmute(values);
        max_val = max_val.max(vals.iter().max().copied().unwrap_or(0)); // Safe fallback, chunk has 4 elements
    }

    // Handle remaining elements
    let remainder_start = (data.len() / 4) * 4;
    if remainder_start < data.len() {
        max_val = max_val.max(data[remainder_start..].iter().max().copied().unwrap_or(0));
        // Safe fallback for remainder
    }
