 ...
 */
 ...
 impl SyscallCompletionEntry {
     ...
     pub fn new(seq: u64, status: SyscallCompletionStatus, result: SyscallResult, user_data: u64) -> Self {
         Self {
             seq,
             status,
             result,
     ...
     pub fn error(seq: u64, error: String, user_data: u64) -> Self {
         let result = SyscallResult::Error { message: error.clone() };
         Self::new(seq, SyscallCompletionStatus::Error(error), result, user_data)
     }
