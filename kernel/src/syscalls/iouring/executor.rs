         ...
         task::spawn_blocking(move || {
             match op {
                ...
                SyscallOpType::Fsync { fd } => {
                 ...
                 SyscallOpType::Send { sockfd, data, flags } => {
                     executor.send(pid, sockfd, &data, flags)
                 }
                 SyscallOpType::Recv { sockfd, size, flags } => {
                     executor.recv(pid, sockfd, size, flags)
                 }
                 SyscallOpType::Accept { sockfd } => {
                     executor.accept(pid, sockfd)
                 }
