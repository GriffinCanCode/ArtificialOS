    ...
    fn handle(&self, pid: Pid, syscall: &Syscall) -> Option<SyscallResult> {
        match syscall {
            Syscall::Mmap { ref path, offset, length, prot, shared } => {
                Some(self.executor.mmap(pid, path, *offset, *length, *prot, *shared))
            }
            Syscall::MmapRead { mmap_id, offset, length } => {
                Some(self.executor.mmap_read(pid, *mmap_id, *offset, *length))
            }
            Syscall::MmapWrite { mmap_id, offset, ref data } => {
                Some(self.executor.mmap_write(pid, *mmap_id, *offset, data))
            }
            Syscall::Msync { mmap_id } => {
                Some(self.executor.msync(pid, *mmap_id))
            }
            Syscall::Munmap { mmap_id } => {
                Some(self.executor.munmap(pid, *mmap_id))
            }
            Syscall::MmapStats { mmap_id } => {
                Some(self.executor.mmap_stats(pid, *mmap_id))
            }
