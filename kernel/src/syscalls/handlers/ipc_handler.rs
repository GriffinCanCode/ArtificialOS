    ...
    fn handle(&self, pid: Pid, syscall: &Syscall) -> Option<SyscallResult> {
        match syscall {
            ...
            Syscall::CreatePipe { reader_pid, writer_pid, capacity } => {
                Some(self.executor.create_pipe(pid, *reader_pid, *writer_pid, *capacity))
            }
            ...
            Syscall::ClosePipe { pipe_id } => {
                Some(self.executor.close_pipe(pid, *pipe_id))
            }
            Syscall::DestroyPipe { pipe_id } => {
                Some(self.executor.destroy_pipe(pid, *pipe_id))
            }
            Syscall::PipeStats { pipe_id } => {
                Some(self.executor.pipe_stats(pid, *pipe_id))
            }

            // Shared memory operations
            Syscall::CreateShm { size } => {
                Some(self.executor.create_shm(pid, *size))
            }
            Syscall::AttachShm { segment_id, read_only } => {
                Some(self.executor.attach_shm(pid, *segment_id, *read_only))
            }
            Syscall::DetachShm { segment_id } => {
                Some(self.executor.detach_shm(pid, *segment_id))
            }
            Syscall::WriteShm { segment_id, offset, ref data } => {
                Some(self.executor.write_shm(pid, *segment_id, *offset, data))
            }
            Syscall::ReadShm { segment_id, offset, size } => {
                Some(self.executor.read_shm(pid, *segment_id, *offset, *size))
            }
            Syscall::DestroyShm { segment_id } => {
                Some(self.executor.destroy_shm(pid, *segment_id))
            }
            Syscall::ShmStats { segment_id } => {
                Some(self.executor.shm_stats(pid, *segment_id))
            }

            // Queue operations
            Syscall::CreateQueue { ref queue_type, capacity } => {
                Some(self.executor.create_queue(pid, queue_type, *capacity))
            }
            Syscall::SendQueue { queue_id, ref data, priority } => {
                Some(self.executor.send_queue(pid, *queue_id, data, *priority))
            }
            Syscall::ReceiveQueue { queue_id } => {
                Some(self.executor.receive_queue(pid, *queue_id))
            }
            ...
            Syscall::CloseQueue { queue_id } => {
                Some(self.executor.close_queue(pid, *queue_id))
            }
            Syscall::DestroyQueue { queue_id } => {
                Some(self.executor.destroy_queue(pid, *queue_id))
            }
            Syscall::QueueStats { queue_id } => {
                Some(self.executor.queue_stats(pid, *queue_id))
            }
