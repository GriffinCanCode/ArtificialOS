    ...
    fn handle(&self, pid: Pid, syscall: &Syscall) -> Option<SyscallResult> {
        match syscall {
            ...
            Syscall::Accept { sockfd } => {
                Some(self.executor.accept(pid, *sockfd))
            }
            Syscall::Connect { sockfd, ref address } => {
                Some(self.executor.connect(pid, *sockfd, address))
            }
            Syscall::Send { sockfd, ref data, flags } => {
                Some(self.executor.send(pid, *sockfd, data, *flags))
            }
            Syscall::Recv { sockfd, size, flags } => {
                Some(self.executor.recv(pid, *sockfd, *size, *flags))
            }
            Syscall::SendTo { sockfd, ref data, ref address, flags } => {
                Some(self.executor.sendto(pid, *sockfd, data, address, *flags))
            }
            Syscall::RecvFrom { sockfd, size, flags } => {
                Some(self.executor.recvfrom(pid, *sockfd, *size, *flags))
            }
            Syscall::CloseSocket { sockfd } => {
                Some(self.executor.close_socket(pid, *sockfd))
            }
            Syscall::SetSockOpt { sockfd, level, optname, ref optval } => {
                Some(self.executor.setsockopt(pid, *sockfd, *level, *optname, optval))
            }
            Syscall::GetSockOpt { sockfd, level, optname } => {
