...
impl SyscallHandler for FileSystemHandler {
    ...
    fn handle(&self, pid: Pid, syscall: &Syscall) -> Option<SyscallResult> {
        match syscall {
            Syscall::ReadFile { ref path } => Some(self.executor.read_file(pid, path)),
            Syscall::WriteFile { ref path, ref data } => {
                Some(self.executor.write_file(pid, path, data))
            }
            Syscall::CreateFile { ref path } => Some(self.executor.create_file(pid, path)),
            Syscall::DeleteFile { ref path } => Some(self.executor.delete_file(pid, path)),
            Syscall::ListDirectory { ref path } => Some(self.executor.list_directory(pid, path)),
            Syscall::FileExists { ref path } => Some(self.executor.file_exists(pid, path)),
            Syscall::FileStat { ref path } => Some(self.executor.file_stat(pid, path)),
            Syscall::MoveFile {
                ref source,
                ref destination,
            ...
            Syscall::GetWorkingDirectory => Some(self.executor.get_working_directory(pid)),
