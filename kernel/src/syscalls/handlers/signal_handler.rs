...
impl SyscallHandler for SignalHandler {
    ...
    fn handle(&self, pid: Pid, syscall: &Syscall) -> Option<SyscallResult> {
        match syscall {
            ...
            Syscall::RegisterSignalHandler { signal, handler_id } => {
                Some(self.executor.register_signal_handler(pid, *signal, *handler_id))
            }
            Syscall::BlockSignal { signal } => {
                Some(self.executor.block_signal(pid, *signal))
            }
            Syscall::UnblockSignal { signal } => {
                Some(self.executor.unblock_signal(pid, *signal))
            }
            Syscall::GetPendingSignals => {
                Some(self.executor.get_pending_signals(pid))
            }
            Syscall::GetSignalStats => {
                Some(self.executor.get_signal_stats(pid))
            }
            Syscall::WaitForSignal { signals, timeout_ms } => {
