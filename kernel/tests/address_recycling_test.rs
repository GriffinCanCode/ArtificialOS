...
fn test_address_recycling() {
    ...
    let addr1 = manager.allocate(1024, pid).expect("Failed to allocate block 1");
    let addr2 = manager.allocate(2048, pid).expect("Failed to allocate block 2");
    let addr3 = manager.allocate(512, pid).expect("Failed to allocate block 3");
    ...
    manager.deallocate(addr2).expect("Failed to deallocate block 2");
    println!("\nDeallocated addr2 (2048 bytes at 0x{:x})", addr2);

    // Allocate a block that fits in the freed space (best-fit should reuse addr2)
    let addr4 = manager.allocate(1024, pid).expect("Failed to allocate block 4");
    println!("\nNew allocation:");
    ...
    assert_eq!(addr4, addr2, "Address should be recycled from freed block (best-fit)");
    ...
    let addr5 = manager.allocate(512, pid).expect("Failed to allocate block 5");
    println!("\nAnother allocation:");
    ...
    // Clean up
    manager.deallocate(addr1).expect("Failed to deallocate addr1");
    manager.deallocate(addr3).expect("Failed to deallocate addr3");
    manager.deallocate(addr4).expect("Failed to deallocate addr4");
    manager.deallocate(addr5).expect("Failed to deallocate addr5");
...
fn test_address_exhaustion_prevented() {
    ...
    for i in 0..10 {
        let addr = manager.allocate(block_size, pid).expect(&format!("Failed allocation {}", i));
        addresses.push(addr);
    ...
    for (i, addr) in addresses.iter().enumerate() {
        manager.deallocate(*addr).expect(&format!("Failed deallocation {}", i));
    }
    ...
    for i in 0..10 {
        let addr = manager.allocate(block_size, pid).expect(&format!("Failed recycled allocation {}", i));
    ...
    let recycled_count = recycled_addresses.iter()
        .filter(|addr| **addr <= original_max)
        .count();

    println!("\nRecycled {} out of {} addresses from free list", recycled_count, recycled_addresses.len());
    assert!(recycled_count > 0, "At least some addresses should be recycled");

    // Clean up
    for addr in recycled_addresses {
        manager.deallocate(addr).expect("Failed cleanup deallocation");
    }
...
fn test_coalescing_adjacent_blocks() {
    ...
    let addr1 = manager.allocate(1000, pid).expect("Failed to allocate block 1");
    let addr2 = manager.allocate(1000, pid).expect("Failed to allocate block 2");
    let addr3 = manager.allocate(1000, pid).expect("Failed to allocate block 3");
    ...
    manager.deallocate(addr1).expect("Failed to deallocate addr1");
    manager.deallocate(addr2).expect("Failed to deallocate addr2");
    manager.deallocate(addr3).expect("Failed to deallocate addr3");

    println!("\nDeallocated all three blocks (should coalesce if adjacent)");

    // Allocate a large block that fits in the coalesced space
    let addr4 = manager.allocate(2500, pid).expect("Failed to allocate large block");
