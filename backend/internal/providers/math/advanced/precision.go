...
func getPrecision(params map[string]interface{}) uint {
	if prec, ok := common.GetNumber(params, "precision"); ok && prec > 0 {
...
func (p *PrecisionOps) PreciseAdd(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	...
	if !ok {
		return common.Failure("numbers parameter required")
	}
	...
	case []interface{}:
		for _, item := range v {
			if str, ok := item.(string); ok {
				...
				return common.Failure("all numbers must be strings for precision arithmetic")
			}
		}
	default:
		return common.Failure("numbers must be array of strings")
	}
	...
	for _, numStr := range numbers {
		...
		if _, ok := num.SetString(numStr); !ok {
			return common.Failure("invalid number format: " + numStr)
		}
...
func (p *PrecisionOps) PreciseSubtract(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	aStr, ok := common.GetString(params, "a")
	if !ok {
		return common.Failure("a parameter required (as string)")
	}

	bStr, ok := common.GetString(params, "b")
	if !ok {
		return common.Failure("b parameter required (as string)")
	}
	...
	if _, ok := a.SetString(aStr); !ok {
		return common.Failure("invalid number format for a")
	}

	b := new(big.Float).SetPrec(prec)
	if _, ok := b.SetString(bStr); !ok {
		return common.Failure("invalid number format for b")
	}
