...
type Math struct {
	// Module instances
	arithmetic  *mathProvider.ArithmeticOps
	trig        *mathProvider.TrigOps
	stats       *mathProvider.StatsOps
	constants   *mathProvider.ConstantsOps
	conversions *mathProvider.ConversionsOps
	precision   *mathProvider.PrecisionOps
	special     *mathProvider.SpecialOps
}

// NewMath creates a modular math provider
func NewMath() *Math {
	ops := &mathProvider.MathOps{}

	return &Math{
		arithmetic:  &mathProvider.ArithmeticOps{MathOps: ops},
		trig:        &mathProvider.TrigOps{MathOps: ops},
		stats:       &mathProvider.StatsOps{MathOps: ops},
		constants:   &mathProvider.ConstantsOps{MathOps: ops},
		conversions: &mathProvider.ConversionsOps{MathOps: ops},
		precision:   &mathProvider.PrecisionOps{MathOps: ops},
		special:     &mathProvider.SpecialOps{MathOps: ops},
	}
}

// Definition returns service metadata with all module tools
func (m *Math) Definition() types.Service {
...
func (m *Math) Execute(ctx context.Context, toolID string, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	switch toolID {
