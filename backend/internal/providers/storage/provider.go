...
// Provider implements persistent key-value storage per app
...
// NewProvider creates a storage provider
...
func (s *Provider) set(ctx context.Context, appID string, params map[string]interface{}) (*types.Result, error) {
...
func (s *Provider) get(ctx context.Context, appID string, params map[string]interface{}) (*types.Result, error) {
	key, ok := params["key"].(string)
	if !ok || key == "" {
		return failure("key parameter required")
	}
...
func (s *Provider) remove(ctx context.Context, appID string, params map[string]interface{}) (*types.Result, error) {
	key, ok := params["key"].(string)
	if !ok || key == "" {
		return failure("key parameter required")
	}
...
func (s *Provider) evictCacheEntries() {
