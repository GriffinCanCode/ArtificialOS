...
// Provider implements comprehensive file system operations
type Provider struct {
	...
	search     *SearchOps
	formats    *FormatsOps
	archives   *ArchivesOps
}

// NewProvider creates a modular filesystem provider
func NewProvider(kernel KernelClient, storagePID uint32, storagePath string) *Provider {
	...
	return &Provider{
		...
		storagePID:  storagePID,
		storagePath: storagePath,
		basic:       &BasicOps{FilesystemOps: ops},
		directory:   &DirectoryOps{FilesystemOps: ops},
		operations:  &OperationsOps{FilesystemOps: ops},
		metadata:    &MetadataOps{FilesystemOps: ops},
		search:      &SearchOps{FilesystemOps: ops},
		formats:     &FormatsOps{FilesystemOps: ops},
		archives:    &ArchivesOps{FilesystemOps: ops},
	}
}

// Definition returns service metadata with all module tools
func (f *Provider) Definition() types.Service {
...
func (f *Provider) Execute(ctx context.Context, toolID string, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	switch toolID {
	// Basic operations
