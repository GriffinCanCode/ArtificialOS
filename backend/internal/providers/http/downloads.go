...
func (d *DownloadsOps) Download(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	...
	filename, _ := GetString(params, "filename", false)
	if filename != "" {
	...
	select {
	case <-ctx.Done():
		return Failure(fmt.Sprintf("download cancelled: %v", ctx.Err()))
	default:
	}

	// Create parent directories if needed
	createDirs := GetBool(params, "create_dirs", true)
	if createDirs {
		dir := filepath.Dir(path)
		if err := os.MkdirAll(dir, 0755); err != nil {
			return Failure(fmt.Sprintf("failed to create directory: %v", err))
		}
	}
	...
	if err != nil {
		...
		os.Remove(path)
		return Failure(fmt.Sprintf("download failed: %v", err))
	}

	// Check HTTP status
	if resp.StatusCode() < 200 || resp.StatusCode() >= 300 {
		os.Remove(path)
		return Failure(fmt.Sprintf("download failed: HTTP %d", resp.StatusCode()))
	}

	// Get file info
	stat, err := os.Stat(path)
	if err != nil {
		return Failure(fmt.Sprintf("failed to stat downloaded file: %v", err))
	}
