...
func (p *ParseOps) ParseJSON(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	jsonStr, err := client.GetString(params, "json", true)
	if err != nil {
		return client.Failure(err.Error())
	}

	if strings.TrimSpace(jsonStr) == "" {
		return client.Failure("json string is empty")
	}

	strict := client.GetBool(params, "strict", false)
	...
	if err := decoder.Decode(&parsed); err != nil {
		return client.Failure(fmt.Sprintf("invalid JSON: %v", err))
	}

	// Check for trailing data
	if decoder.More() {
		return client.Failure("JSON contains trailing data")
	}
...
func (p *ParseOps) ParseXML(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	xmlStr, err := client.GetString(params, "xml", true)
	if err != nil {
		return client.Failure(err.Error())
	}

	if strings.TrimSpace(xmlStr) == "" {
		return client.Failure("xml string is empty")
	}
	...
	if err := xml.Unmarshal([]byte(xmlStr), &parsed); err != nil {
		...
		if err2 := xml.Unmarshal([]byte(xmlStr), &parsedArray); err2 != nil {
			return client.Failure(fmt.Sprintf("invalid XML: %v", err))
		}
...
func (p *ParseOps) StringifyJSON(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	...
	pretty := client.GetBool(params, "pretty", false)
	...
	if err != nil {
		return client.Failure(fmt.Sprintf("failed to stringify: %v", err))
	}
