...
func (u *URLOps) BuildURL(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	base, err := GetString(params, "base", true)
	if err != nil {
	...
	if err != nil {
		return Failure(fmt.Sprintf("invalid base URL: %v", err))
	}
	...
	if queryParams := GetMap(params, "params"); queryParams != nil {
		q := parsedURL.Query()
	...
	if fragment, _ := GetString(params, "fragment", false); fragment != "" {
...
func (u *URLOps) ParseURL(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	...
	if err != nil {
		return Failure(fmt.Sprintf("invalid URL: %v", err))
	}
...
func (u *URLOps) JoinPath(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	...
	segments := GetArray(params, "segments")
	if len(segments) == 0 {
		return Failure("segments array required")
	}
	...
	parsedURL, err := url.Parse(base)
	if err != nil {
		return Failure(fmt.Sprintf("invalid base: %v", err))
	}
...
func (u *URLOps) EncodeQuery(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	queryParams := GetMap(params, "params")
	if queryParams == nil {
		return Failure("params object required")
	}
...
func (u *URLOps) DecodeQuery(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	queryStr, err := GetString(params, "query", true)
	if err != nil {
		return Failure(err.Error())
	}
	...
	if err != nil {
		return Failure(fmt.Sprintf("invalid query string: %v", err))
	}
