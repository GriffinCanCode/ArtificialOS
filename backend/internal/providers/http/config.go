...
func (c *ConfigOps) SetHeader(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	...
	value, err := GetString(params, "value", false)
	if err != nil {
		return Failure(err.Error())
	}
...
func (c *ConfigOps) SetTimeout(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	seconds, err := GetNumber(params, "seconds", true)
	if err != nil {
		return Failure(err.Error())
	}

	if seconds <= 0 {
		return Failure("seconds must be positive number")
	}
...
func (c *ConfigOps) SetAuth(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	authType, err := GetString(params, "type", true)
	if err != nil {
		return Failure(err.Error())
	}
	...
	switch authType {
	case "basic":
		username, err := GetString(params, "username", true)
		if err != nil {
			return Failure("username required for basic auth")
		}

		password, err := GetString(params, "password", true)
		if err != nil {
			return Failure("password required for basic auth")
		}

		c.Client.SetBasicAuth(username, password)

	case "bearer":
		token, err := GetString(params, "token", true)
		if err != nil {
			return Failure("token required for bearer auth")
		}

		c.Client.SetBearerAuth(token)

	case "custom":
		header, err := GetString(params, "header", true)
		if err != nil {
			return Failure("header required for custom auth")
		}

		c.Client.SetCustomAuth(header)

	default:
		return Failure(fmt.Sprintf("invalid auth type: %s (must be: basic, bearer, or custom)", authType))
	}
