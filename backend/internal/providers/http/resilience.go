...
func (r *ResilienceOps) SetRetry(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	maxRetries, err := GetNumber(params, "max_retries", true)
	if err != nil {
		return Failure(err.Error())
	}

	if maxRetries < 0 || maxRetries > 10 {
		return Failure("max_retries must be between 0 and 10")
	}

	minWait := 1.0
	if mw, err := GetNumber(params, "min_wait_seconds", false); err == nil && mw > 0 {
		minWait = mw
	}

	maxWait := 30.0
	if mw, err := GetNumber(params, "max_wait_seconds", false); err == nil && mw > 0 {
		maxWait = mw
	}

	if minWait > maxWait {
		return Failure("min_wait_seconds cannot exceed max_wait_seconds")
	}
...
func (r *ResilienceOps) SetRateLimit(ctx context.Context, params map[string]interface{}, appCtx *types.Context) (*types.Result, error) {
	rps, err := GetNumber(params, "requests_per_second", true)
	if err != nil {
		return Failure(err.Error())
	}

	if rps < 0 {
		return Failure("requests_per_second cannot be negative")
	}
