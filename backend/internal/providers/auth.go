...
// NewAuth creates an auth provider
func NewAuth(kernel KernelClient, storagePID uint32, storagePath string) *Auth {
	return &Auth{
		kernel:      kernel,
...
func (a *Auth) Definition() types.Service {
...
func (a *Auth) login(params map[string]interface{}) (*types.Result, error) {
	username, ok := params["username"].(string)
	if !ok || username == "" {
		return failure("username required")
	}
...
func (a *Auth) logout(params map[string]interface{}) (*types.Result, error) {
...
func (a *Auth) verify(params map[string]interface{}) (*types.Result, error) {
	token, ok := params["token"].(string)
	if !ok || token == "" {
		return failure("token required")
	}
...
func (a *Auth) getUser(params map[string]interface{}) (*types.Result, error) {
	token, ok := params["token"].(string)
	if !ok || token == "" {
		return failure("token required")
	}
