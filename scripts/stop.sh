...
if [ ! -z "$KERNEL_PORT_PID" ]; then
    ...
    pkill -f "ai_os_kernel" 2>/dev/null && echo "   ✅ Kernel stopped" || echo "   ℹ️  Kernel not running"
fi
...
if [ ! -z "$BACKEND_PORT_PID" ]; then
    ...
    pkill -f "backend/bin/server" 2>/dev/null && echo "   ✅ Backend stopped" || echo "   ℹ️  Backend not running"
fi

# Stop UI
pkill -f "vite" 2>/dev/null && echo "   ✅ UI stopped" || echo "   ℹ️  UI not running"
